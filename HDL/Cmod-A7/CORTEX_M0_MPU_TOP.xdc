...
# 12 MHz Clock Signal
set_property -dict { PACKAGE_PIN L17   IOSTANDARD LVCMOS33 } [get_ports { ExtClk }]; #IO_L12P_T1_MRCC_14 Sch=gclk
create_clock -add -name ExtClk_pin -period 83.33 -waveform {0 41.66} [get_ports {ExtClk}];
#create_clock -add -name ExtClk_pin -period 100.00 -waveform {0 50.00} [get_ports {ExtClk}];
...
set_property -dict { PACKAGE_PIN J19   IOSTANDARD LVCMOS33 } [get_ports { SWCLK }]; #IO_L6N_T0_D08_VREF_14 Sch=ja[9]
set_property -dict { PACKAGE_PIN K18   IOSTANDARD LVCMOS33 } [get_ports { SWDIO }]; #IO_L8N_T1_D12_14 Sch=ja[10]

set_property CLOCK_DEDICATED_ROUTE FALSE [get_nets SWCLK]
create_clock -add -name SWCLK_pin -period 1000 -waveform {0 500} [get_ports {SWCLK}];

set_clock_groups -asynchronous \
    -group [get_clocks {ExtClk_pin}] \
	  -group [get_clocks {SWCLK_pin}] \
    -group [get_clocks {CLK_PLL}]
